...
// ---- module: builtin-materialize (plugins/workflow-host/builtin-materialize.js) ----
// ============================================================================
...
function registerWebRoutes(ctx, registry) {
  ...
  webserver.register({
    ...
    handler: async (req, res) => {
      ...
      // Iter-39 临时诊断：客户端探针回传（写服务日志 journalctl；诊断结束即删）
      if (req.method === 'POST' && pathname === '/wf/debug-probe') {
        let b36 = ''
        req.on('data', (c) => { b36 += c })
        req.on('end', () => {
          try { console.log('[wf-gate-probe][client] ' + String(b36).slice(0, 500)) } catch (e36) {}
