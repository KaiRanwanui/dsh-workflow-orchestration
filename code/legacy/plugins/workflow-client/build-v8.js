...
const payload = {
  ...
  name: 'WF DAG v8 - fixed',
  purpose: 'Fix all runtime bugs: c is not defined, RUNNING counts, opacity/stat/cursor, arrow len',
  code: { host, client },
}
const json = JSON.stringify(payload, null, 2)
fs.writeFileSync(path.join(__dirname, '_payload-v8.json'), json, 'utf8')
console.log('Payload: ' + json.length + ' bytes')
const p2 = JSON.parse(json)
try { new Function(p2.code.client); console.log('Client OK') } catch(e) { console.log('Client ERR: ' + e.message) }
try { new Function(p2.code.host); console.log('Host OK') } catch(e) { console.log('Host ERR: ' + e.message) }