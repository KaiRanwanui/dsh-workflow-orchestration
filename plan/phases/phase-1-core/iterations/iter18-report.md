...
**Iter-20 归档存储 + 管理（Host）**：显式归档（`archive_<state>` 移出池）+ `listArchive`/`downloadArchive`(zip)/`deleteArchive` 工具与路由；归档后会话 BOUND→DONE（技术方案 `../../../design/workflow-lifecycle-design.md` §7）。
