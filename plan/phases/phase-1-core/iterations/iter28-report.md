...
- **配套**：`../../../architecture/architecture-decisions.md` §8（rc2 persona 内联系统限制与单源化决策）
...
- **系统限制（rc2 无法解耦）**：`@deepseek-ai/dsh-persona@0.1.1-rc.2` Config 仅 `text: string`（无 file/path 引用）——persona 必须内联 composition，与 md 源文档双写。完整决策与 **DSH 升级检查项**记档 `../../../architecture/architecture-decisions.md` §8。
