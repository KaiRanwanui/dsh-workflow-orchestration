...
- **状态**：✅ 完成关闭（2026-09-17，用户验收通过：多错误合并呈现复验确认；两处验收缺陷修复确认）
- **阶段**：阶段 4（现有功能修复/补全）
- **版本**：host `v0.26.5 → v0.26.8`（0.26.6 主交付 / 0.26.7 验收缺陷两处 / 0.26.8 多错误合并呈现；单包）
...
## 13. 补记：depends-on 多选 chips（v0.26.11，2026-09-17，用户要求）

Iter-36 验收反馈：depends-on 自由文本输入改为**从现有任务定义多选 id 的 chips 交互**（排除自身；失效引用显示为可删除警示项；引用存在性由语义关口兜底）。Host 无改动（保存通道/校验复用），纯 Client 展示层。
