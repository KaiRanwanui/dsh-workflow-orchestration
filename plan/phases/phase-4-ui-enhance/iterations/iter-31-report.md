...
| 真机 reset（步骤 1） | ⏳ 待用户验收 | 重启 dsh 后：跑一个实例 → Stop → Reset → 应停留 PENDING 且 Agent 不自动 begin；面板 Start 正常启动 |
| 真机 stopHint（步骤 2） | ⏳ 待用户验收 | 主会话空闲+子会话在跑场景：不再出现黄色提示条；停止两通道复验仍全停 |
...
| 遗留 | 去向 |
|---|---|
| 真机两项验收（重启 dsh 后） | 用户 GUI 确认后本迭代关闭 |
