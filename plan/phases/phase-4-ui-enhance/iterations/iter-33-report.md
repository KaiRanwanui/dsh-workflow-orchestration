...
- **状态**：✅ 完成关闭（2026-09-17，用户真机验收通过：#11 reset 回归通过不再复发、#9 绑定保持回归通过不再复发、「（离线）」展示已清除）
- **阶段**：阶段 4（现有功能修复）
- **版本**：host `v0.25.2 → v0.26.2`（v0.26.0 主交付 / v0.26.1 字段映射修复 / v0.26.2 U3 展示修复；单包）
- **测试**：581 单测全绿（+用例 33 孤儿判定对照 3 断言；c32/c21 等既有断言无回归）+ 产物级验证通过
- **提交**：`91d1c75`（主交付）/ `53b03ad`（v0.26.1 字段映射修正）/ `df06f27`（v0.26.2 U3）
...
| 真机 ①绑定保持 | ✅ 用户验收（回归通过，不再复发） | 重启 dsh 后：已绑定实例保持绑定，管理列表不再批量「未绑定」 |
| 真机 ②面板 Reset | ✅ 用户验收（回归通过，不再复发） | verify-empty-items 实例 Reset 成功、停留 PENDING；inputs 恢复模板初始态 |
| 真机 ③采纳关口 | ✅ 随收尾关闭 | 构造不完整实例 → 采纳被拒 + 结构化原因（用户收尾指令整体确认） |
| 真机 ④CREATED 明示 | ✅ 随收尾关闭 | CREATED 实例面板显示状态卡、可归档清理（用户收尾指令整体确认） |
...
| U3 管理列表「（离线）」后缀清除（用户拍板，提前于 Iter-39 落地） | ✅ v0.26.2 已修（展示层单行删除） |
