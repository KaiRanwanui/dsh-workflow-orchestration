...
| **Iter-38** | 前台 | params 影响分析与单轨化 | ✅ 已完成关闭（host v0.26.20；插队迭代，方案=iter-38-analysis.md 用户拍板） | 1.5 天 |
| **Iter-39** | UI | 页签动态门控 | 非编排会话完全隐藏 Workflow 页签（现状=渲染占位文案仍占 tab）；契约探针已明：list slot + register 返回 disposer + markDirty 刷页签 | 0.5 天 |
| **Iter-40** | UI | RUNNING 运行视觉 | RUNNING 脉冲（呼吸动画）+ 自动跟随（视口外才滚、每指纹一次）+ 门禁角点执行后消失修复（补验 U1） | 0.75 天 |
| **Iter-41** | UI | 节点详情与文件预览 | 节点详情面板 + 文件内容预览（**方案待 Iter-35 全文编辑效果评估后确定**） | 1 天 |
| **Iter-42** | UI | 交互直观性优化 | 定义生效引导（保存→Reset/begin 提示与一键流）、DAG 随定义刷新的直观呈现（来源：用户验证反馈 09-17） | 待定 |
| **Iter-43** | UI | 主题适配 | 173 处 style → 13 CSS token（含 39~42 新增 UI） | ✅ 已完成关闭（host v0.26.50） |

> **执行结果对照（2026-09-20 阶段收尾）**：Iter-31~43 全部验收关闭，最终基线 host v0.26.50/602 单测。执行中的方向调整以各迭代报告为准：Iter-38 由「影响分析」插队升格为 params 单轨化（/wf/instance-params 退役）；Iter-42 由「定义生效引导一键流」重定案为 **DAG 数据源重构**（结构=instance.yaml 所见即所得+状态叠加，一键流废弃）；Iter-43 交付含浅色主题四项微调与选中描边三型统一。详见 iterations/ 各报告。

> 版本规则（用户 09-17 定稿）：**本阶段第二段恒为 26；每次发行（迭代主交付或补丁）第三段 +1；不预设某迭代发哪个版本**。已发：Iter-31=v0.24.0（规则前）、Iter-32=v0.25.0~2（规则前）、Iter-33=0.26.0~2、Iter-34=0.26.3（0.27.x 误编已纠正）。
