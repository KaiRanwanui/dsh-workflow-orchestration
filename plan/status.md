...
| **阶段 4 · 现有功能修复**（Iter-31 ~ 41） | 2026-09-16 ~ | 0.1.5-rc.2（不变） | **host v0.26.2**（单包形态延续；进行中） | 581 单测全绿（含用例 32/33） | 🚧 进行中 |
...
| Iter-34 | 门禁真实执行（引擎软强制 + pendingGates + gateNote 反馈链；复用 verify-gate 模板） | 🚧 编码完成（host v0.26.3），待真机验收 |
