...
| **阶段 4 · 现有功能修复**（Iter-31 ~ 41） | 2026-09-16 ~ | 0.1.5-rc.2（不变） | **host v0.25.2**（单包形态延续；进行中） | 578 单测全绿（含用例 32 组依赖） | 🚧 进行中 |
...
| Iter-33 | 实例完整性与采纳关口（含**孤儿回收误判 #9**：isSessionLive 驻留语义致存活会话实例被批量误解绑） | 🚧 编码完成（host v0.26.0），待真机验收 |
| Iter-34 | 门禁真实执行（诊断先行；复用 Iter-32 verify-gate 模板——补验已证门禁链路通，聚焦 #8 场景） | 待启动 |
| Iter-35 ~ 37 | 定义全文编辑 / 编辑器表单补全（含 U2 模板下拉名称+说明）/ 全局参数编辑 | 待启动 |
