...
| **阶段 4 · 现有功能修复**（Iter-31 ~ 41） | 2026-09-16 ~ | 0.1.5-rc.2（不变） | **host v0.26.5**（单包形态延续；进行中） | 588 单测全绿（含用例 32/33/34） | 🚧 进行中 |
...
| Iter-35 | 定义全文编辑（YAML 源码模式两态切换 + 创建弹窗去 yaml 编辑 + 技能全文只读浏览） | 🚧 编码完成（host v0.26.6），待真机验收 |
