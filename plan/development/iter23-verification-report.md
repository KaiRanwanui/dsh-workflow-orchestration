...
- **状态**：代码完成（host v0.12.0 / client v0.6.0），单测 220 全绿，待部署 + 手测
...
**预期**：
- [ ] 步骤 3：面板实例状态 **STOPPED**（非 RUNNING 残留）
- [ ] metadata.json `stopReason` = `"user-stop"`
- [ ] 正在跑的后台子会话**秒级消失**（面板会话树/apiProxy.subagents.list 无 running 条目）
- [ ] 步骤 4：agent 可回复，但**工作流不推进**（通知免疫：P4 忽略 + stopReason=user-stop 永不自动恢复）
- [ ] 步骤 5：Start **被拒**（闭环不变式：Stop 后需 Resume）
...
**预期**：
- [ ] 步骤 2：面板出现提示条"编排会话空闲等待中：会话内的停止按钮此刻无效。后台任务执行中——要停止工作流请点面板 Stop"
- [ ] 步骤 3：实例保持 RUNNING（Case I 零痕迹，点击无效果——原理性边界，非缺陷）
- [ ] 步骤 4-5：子会话秒级停止，实例 STOPPED，`stopReason=user-stop`
- [ ] 子会话自然跑完后的对照：提示条消失，实例自然收敛 STOPPED（`stopReason=session-idle`）

## V3 回归：三条既有停止路径不变

- [ ] **面板 Stop**（agent 干活时点）→ STOPPED(user-stop)+级联，行为与 Iter-SUBA T 手测一致
- [ ] **自然语言"请停止工作流"** → agent 调 workflow_stop → 同上
- [ ] **自然收敛**（子会话全部跑完、agent idle）→ STOPPED(session-idle) + 主会话活跃时自动 resume（P2）
- [ ] workflow_resume 清 stopReason，可续跑
...
（待手测后回填：通过项打勾；问题按"现象/引擎日志/metadata/处置"记录）
